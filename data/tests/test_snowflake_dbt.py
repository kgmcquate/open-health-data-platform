"""The dbt project surfaces as Dagster assets under `snowflake/`, wired into the
ingestion lineage."""
...
def test_models_are_snowflake_prefixed_and_grouped_by_layer() -> None:
    """`snowflake/*` now also contains dbt's raw *sources* — the model check
...
def test_healthdata_gov_bridges_the_dlt_table_to_a_snowflake_raw_asset() -> None:
    """`HealthDataGovDataset._snowflake_raw_spec()` (healthdata_gov/component.py)
    puts an unexecutable `snowflake/RAW/<schema>/<table>` asset downstream of
