"""Offline checks for the config-driven CMS ingestion (ADR-0026).
...
assets/jobs/schedules. Mirrors ``test_socrata_components.py``'s checks, minus
the per-domain parametrization (there is exactly one CMS source) and the
Socrata-specific bits (``ColumnSpec``/column schema, ``socrata_id``).
"""
...
from ohdp_ingestion.cms import CADENCES, DatasetConfig

SOURCE = "cms"
_DEFS_FILE = (
    Path(__file__).resolve().parents[1] / "src/ohdp_orchestration/defs/cms/datasets/defs.yaml"
)
...
def test_scraper_has_run() -> None:
    assert _documents(), "run scripts/scrape_cms.py first"


def test_every_instance_is_a_valid_dataset_component() -> None:
    for where, doc in _documents():
        assert doc["type"].endswith("CMSDataset"), where
...
def test_catalog_assets_carry_metadata() -> None:
    ...
    for _, cfg in _configs():
        ...
        assert node.metadata["cms_dataset_id"] == cfg.id
...
def test_at_least_one_dataset_is_enabled() -> None:
    """The hand-picked starter set (ADR-0026) — catches an accidental
    all-disabled scrape rewrite."""
