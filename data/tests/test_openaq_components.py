"""Offline checks for the config-driven OpenAQ ingestion.
...
assets/jobs/schedules. Mirrors ``test_cms_components.py``'s checks — minus
``test_scraper_has_run``/its "run the scraper" framing, since OpenAQ's
``datasets/defs.yaml`` is hand-maintained, not generated (see
``ohdp_orchestration.defs.openaq``'s docstring).
"""
...
from ohdp_ingestion.openaq import CADENCES, DatasetConfig

SOURCE = "openaq"
_DEFS_FILE = (
    Path(__file__).resolve().parents[1] / "src/ohdp_orchestration/defs/openaq/datasets/defs.yaml"
)
...
def test_defs_file_has_at_least_one_instance() -> None:
    assert _documents(), "datasets/defs.yaml is hand-maintained -- see defs.openaq's docstring"


def test_every_instance_is_a_valid_dataset_component() -> None:
    for where, doc in _documents():
        assert doc["type"].endswith("OpenAQDataset"), where
