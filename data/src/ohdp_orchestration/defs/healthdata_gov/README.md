...
| **raw-layer asset** | `lakehouse/raw/healthdata_gov/<raw_table>` | `lakehouse_raw` | `iceberg` | runless event | `enabled: true` |

`source -> ingestion -> lakehouse/raw/* -> (dbt clean → core → marts)`. The
