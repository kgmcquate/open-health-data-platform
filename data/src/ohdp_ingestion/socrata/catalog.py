...
schema, the declared update cadence, and popularity counters. It is the same
API on every Socrata host, so this module is domain-agnostic — the caller
passes a :class:`~ohdp_ingestion.socrata.domain.SocrataDomain` (ADR-0018).
...
def _parse_result(result: dict[str, Any], socrata: SocrataDomain) -> CatalogDataset | None:
    ...
    return CatalogDataset(
        ...
        landing_page=result.get("permalink") or socrata.landing_page(dataset_id),
...
def iter_catalog(
    ...
    *,
    ...
    """Yield every catalog entry for ``socrata``'s domain, newest change first.
    ...
    with http_client(CATALOG_BASE_URL) as client:
        while True:
            params: dict[str, str | int] = {
                "domains": socrata.domain,
                "search_context": socrata.domain,
                "only": only,
                "order": "updatedAt",
                "limit": _PAGE,
                "offset": offset,
            ...
            for result in results:
                parsed = _parse_result(result, socrata)
                if parsed is None:
                    continue
                yield parsed
                yielded += 1
