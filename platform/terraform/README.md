...
| `digitalocean_kubernetes_node_pool` `np_services` | `node-role: services` — 1× `s-4vcpu-8gb`, always on: hub-api, Cube, CubeStore, Postgres; OpenMetadata, OpenSearch |
| `digitalocean_kubernetes_node_pool` `np_4_cpu_8gb` | `node-role: heavy` — `s-4vcpu-8gb`, hand-scaled (`node_count` is ignored): Dagster + its run pods, its oauth2-proxy, gqlproxy and dagster-monitoring |
...
| `snowflake_external_volume` / `snowflake_database` | The lakehouse: S3-backed external volume and the RAW/CLEAN/CURATED catalog databases |
| `snowflake_schema` | Every namespace in it — `RAW.<SOURCE>`, `CLEAN.STG_<SOURCE>`, `CURATED.CORE`, `CURATED.<MART>` from `lakehouse_sources` / `lakehouse_marts`. Horizon's Iceberg REST catalog doesn't implement namespace creation for external engines, so dbt and dlt can't open these themselves (ADR-0021) |
| `snowflake_grant_privileges_to_account_role` | The pipeline role's grants on those databases and namespaces: ALL on the schemas/tables Terraform creates, FUTURE on whatever the pipeline creates inside them afterwards, and on the external volume |
