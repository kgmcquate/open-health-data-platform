...
charts/
  ...
  polaris-console/      our chart — the Apache Polaris console UI (ADR-0010).
                        Upstream ships no Helm repo or image; the image is built
                        by .github/workflows/build-polaris-console.yml
values/
...
make install    # opensearch, openmetadata, polaris, polaris-console, proxy, dagster
...
The Polaris console UI (`polaris-console`) is internal-only — no ingress. Reach
it with `make console` (port-forwards the console on :8080 and the Polaris API on
:8181) and log in with the `ohdp` root principal from `polaris-principal`. Its
image is not on the `main` build path: run the **Build Polaris console image**
workflow (or bump `POLARIS_TOOLS_REF` in it) and keep `image.tag` in
`charts/polaris-console/values.yaml` in sync.
