...
# Polaris (ADR-0010): `polaris-persistence` is generated by platform-base from
# postgres-secret. `polaris-bootstrap` holds the realm root principal for the
# chart's schema-bootstrap init container — "<realm>,<clientId>,<clientSecret>",
# the same principal as OHDP_ICEBERG_CREDENTIAL ("<id>:<secret>"):
kubectl -n data create secret generic polaris-bootstrap \
  --from-literal=credential="ohdp,<polaris client-id>,<client-secret>"
# `make polaris` then installs polaris/polaris (the init container creates the
# schema + `ohdp` realm) and creates the `ohdp` catalog via the management API.
# See platform/scripts/polaris-bootstrap.sh.
...
its own schema + `ohdp` realm from an init container (needs the
`polaris-bootstrap` secret); `make polaris`'s post-install catalog step reads
`OHDP_ICEBERG_CREDENTIAL` (env), falling back to the `ohdp-pipeline-secrets`
secret.
