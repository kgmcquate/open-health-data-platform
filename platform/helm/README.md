...
# postgres-secret — nothing to create by hand. `make polaris` then runs the
# admin-tool bootstrap (schema + `ohdp` realm, root principal ==
# OHDP_ICEBERG_CREDENTIAL), installs polaris/polaris, and creates the `ohdp`
# catalog via the management API. See platform/scripts/polaris-bootstrap.sh.
...
migration job fails; Polaris needs its schema bootstrapped before the server
boots; `make install` sequences this for you. `make polaris` reads the realm
root principal from `OHDP_ICEBERG_CREDENTIAL` (env), falling back to the
`ohdp-pipeline-secrets` secret created above.
