...
(ADR-0016) — so Open WebUI cannot talk to the semantic layer without one. This
module is that server, and it is deliberately *thin*: every tool delegates to
`ohdp_agent.cube.CubeClient`, the same code path the in-process chat agent uses.
...
with no Ingress: the wall in front of Open WebUI is the access control, and this
server trusts its caller.
...
Transport is streamable HTTP at `/mcp`, which is what Open WebUI's native MCP
client speaks.
"""
